...
function map_Type__Path<Annotation>($: g_in.T.Type__Path<Annotation>, $x: {
    ...
    return pl.cc($, ($) => {
        const temp__limport: g_out.T.Type__Path._limport<Annotation> = pl.cc($['import'], ($) => pl.optional(
            $,
            ($): g_out.T.Type__Path._limport<Annotation> => [true, $],
            () => [false],
        ))
        const temp_global__type: g_out.T.Type__Path.global__type<Annotation> = pl.cc($['global type'], ($) => pl.cc($, ($) => {
            const constraint: pt.OptionalValue<g_out.T.Type__Library.global__types.D<Annotation>> = [false]
            return {
                'annotation': $.annotation,
                'constraint': constraint,
        ...
        const temp_path: g_out.T.Type__Path.path<Annotation> = pl.cc($['path'], ($) => $.map(($) => pl.cc($, ($): g_out.T.Type__Path.path.A<Annotation> => {
            switch ($[0]) {
                case 'dictionary': return pl.ss($, ($) => ['dictionary', pl.cc($, ($) => {
                ...
                case 'group': return pl.ss($, ($) => ['group', pl.cc($, ($) => {
                    ...
                    return {
                        ...
                        'constraint': constraint,
                        'type': pl.cc($.type, ($) => pl.cc($, ($) => {
                            const temp_property: g_out.T.Type__Path.path.A.group._ltype.property<Annotation> = pl.cc($['property'], ($) => pl.cc($, ($) => {
                                const constraint: pt.OptionalValue<g_out.T.Type.group.properties.D<Annotation>> = [false]
                                return {
                                    'annotation': $.annotation,
                                    'constraint': constraint,
                ...
                case 'tagged union': return pl.ss($, ($) => ['tagged union', pl.cc($, ($) => {
                    ...
                    return {
                        ...
                        'constraint': constraint,
                        'type': pl.cc($.type, ($) => pl.cc($, ($) => {
                            const temp_option: g_out.T.Type__Path.path.A.tagged__union._ltype.option<Annotation> = pl.cc($['option'], ($) => pl.cc($, ($) => {
                                const constraint: pt.OptionalValue<g_out.T.Type.tagged__union.options.D<Annotation>> = [false]
                                return {
                                    'annotation': $.annotation,
                                    'constraint': constraint,

...
function map_A__tagged__union__with__result<Annotation>(
    ...
    return pl.cc($, ($) => {
        const $p_tu: g_out.T.A__tagged__union__with__result.tu<Annotation> = pl.cc($['tu'], ($) => pl.cc($, ($): g_out.T.A__tagged__union__with__result.tu<Annotation> => {
            switch ($[0]) {
                case 'a': return pl.ss($, ($) => pl.cc($, ($) => { //option with result
                    ...
                    switch ($[0]) {
                        case 'call': return pl.ss($, ($) => ['call', pl.cc($, ($) => {
                            ...
                            return {
                                ...
                                'content': pl.cc($.content, ($) => pl.cc($, ($) => {
                                    const $p_type__arguments: g_out.T.Address__Selection__Tail.content.content.O.step.call.content.type__arguments<Annotation> = pl.cc($['type arguments'], ($) => map_Type__Arguments<Annotation>(
                                        $,
                                        tempoptional/*3*/(
                                            $v_namespace,
                                            ($) => [true, $],
                                    ...
                                    const $v_type__arguments: pt.OptionalValue<g_out.T.Address__Selection__Tail.content.content.O.step.call.content.type__arguments<Annotation>> = [true, $p_type__arguments]
                                    const $p_arguments: g_out.T.Address__Selection__Tail.content.content.O.step.call.content.arguments<Annotation> = pl.cc($['arguments'], ($) => $.__mapWithKey(($, key) => pl.cc($, ($) => {
                                    ...
                                    const $v_arguments: pt.OptionalValue<g_out.T.Address__Selection__Tail.content.content.O.step.call.content.arguments<Annotation>> = [true, $p_arguments]
                                    return {
...
function map_Expression<Annotation>(
    ...
    $v_namespace: pt.OptionalValue<g_out_typesystem.T.Type<Annotation>>,
    $v_stack: pt.OptionalValue<g_out.T.Variables<Annotation>>,
...
function map_Type__Selection<Annotation>(
    ...
    const content = pl.cc($, ($) => {
        ...
        const $p_type__of__referenced__type: g_out.T.Type__Selection.content.type__of__referenced__type<Annotation> = pl.cc($['type of referenced type'], ($) => pl.cc($, ($): g_out.T.Type__Selection.content.type__of__referenced__type<Annotation> => {
            switch ($[0]) {
                case 'namespace': return pl.ss($, ($) => pl.cc($, ($) => { //option with result
                    const content: g_out.T.Type__Selection.content.type__of__referenced__type.content.namespace<Annotation> = pl.cc($, ($) => {
                        const $v_referenced__namespace: g_out_typesystem.T.Namespace.D._ltype.namespace<Annotation> = pl.cc($, ($) => {
                ...
                case 'type definition': return pl.ss($, ($) => pl.cc($, ($) => { //option with result
                    const content: g_out.T.Type__Selection.content.type__of__referenced__type.content.type__definition<Annotation> = pl.cc($, ($) => {
                        const $v_referenced__type__definition: g_out_typesystem.T.Namespace.D._ltype.type__definition<Annotation> = pl.cc($, ($) => {
                            const x: pt.OptionalValue<g_out_typesystem.T.Namespace.D._ltype<Annotation>> = tempoptional/*3*/(
                                $v_referenced__type,
                                ($) => tempoptional/*1*/(
                                    $.constraint,
    ...
    return $.__mapWithKey(($, key) => pl.cc($, ($) => {
        const $p__ltype: g_out.T.Variables.D._ltype<Annotation> = pl.cc($['type'], ($) => pl.cc($, ($): g_out.T.Variables.D._ltype<Annotation> => {
            switch ($[0]) {
                ...
                case 'parameter': return pl.ss($, ($) => pl.cc($, ($) => { //option with result
